...
const items: Item[] = [
  {
    ...
    name: 'Cucumber',
    price: 12,
    description: 'Cucumber is a summer vegetable, with elongate shape and 15cm long. Its skin is of a green colour, turning into yellow in maturation. At present, it is found in the European markets all over the year. Fresh or pickled cucumbers are also available.',
    complete: false,
    updatedAt: Date.now(),
  },
  {
...
export const addItem = function(req: Request, res: Response) {
  let data = {
    ...
    complete: req.body.data.complete,
    updatedAt: Date.now(),
  }
  items.push(data);
  id++;
